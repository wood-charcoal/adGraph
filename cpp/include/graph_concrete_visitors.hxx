...
namespace nvgraph
{
                                                    ...
                                                    Vector<T, Allocator<T>> &out)
  {
    ...
    thrust::inclusive_scan(output_indices.begin(),
                           output_indices.end(),
                           output_indices.begin(),
                           thrust::maximum<difference_type>());

    // gather input values according to index array (output = first2[output_indices])
    Iterator output_end = output;
  ...
  struct MinLeftRightPlusValue
  {
    ...
    explicit MinLeftRightPlusValue(ValT delta) : delta_(delta)
    {
    }

    __host__ __device__
        ValT
        operator()(ValT left, ValT right)
    {
  ...
  struct Filter
  {
    ...
    explicit Filter(VectorR &src) : m_src(&src[0])
    {
    }
    __host__ __device__
        RetT
        operator()(const IndexT &k)
    {
      return m_src[k];
    }
  ...
  struct CleanFctr
  {
    explicit CleanFctr(Container &used) : m_used(&used[0])
    {
    }
    __host__ __device__ bool operator()(const IndexT &k)
    {
      return (m_used[k] == 0);
    }
  ...
  struct ValueUpdater
  {
    ...
    struct ValFiller
    {
      explicit ValFiller(VectorV &v_src) : m_s(&v_src[0])
      {
      }
  ...
  struct Offsets2RowIndex
  {
    ...
    Offsets2RowIndex(VectorB &hash_rows,
                     ...
                     VectorI &J,
                     VectorI &K,
                     VectorB &U) : m_hash_sz(hash_rows.size()),
  ...
  struct NzCounter
  {
    ...
    explicit NzCounter(Container &row_ptr) : m_row_ptr(&row_ptr[0])
    {
    }
  ...
  struct HashFctr
  {
    ...
    explicit HashFctr(Container &hash_src) : m_hash(&hash_src[0])
    {
    }
    __host__ __device__
        IndexT
        operator()(const IndexT &src_elem)
    {
      IndexT hit(1);
      m_hash[src_elem] = hit;
      return hit;
  ...
  struct Hasher
  {
    ...
    explicit Hasher(Container &hash_src) : m_hash(&hash_src[0])
    {
    }
    __host__ __device__
        IndexT
        operator()(IndexT i, IndexT v)
    {
      m_hash[v] = i;
      return v;
    }
  ...
  struct SubGraphExtractorFunctor
  {
    ...
    //
    SubGraphExtractorFunctor(const VectorI &eSub, bool /*unused*/) : edgeSubset(eSub),
                                                                     is_vertex_extraction(false)
    {
    }

    explicit SubGraphExtractorFunctor(const VectorI &vSubset) : vertexSubset(vSubset),
  ...
  template <typename VectorI,
            typename VectorV>
  struct SubGraphExtractorVisitor : VisitorBase,
                                    ...
                                    Visitor<MultiValuedCsrGraph<typename VectorI::value_type, typename VectorV::value_type>>
  {
    ...
    SubGraphExtractorVisitor(CsrGraph<IndexType_> &graph,
                             const VectorI &vSub,
                             hipStream_t stream) : row_ptr_(graph.get_raw_row_offsets(), graph.get_raw_row_offsets() + graph.get_num_vertices() + 1),
  ...
  struct BoundValidator
  {
    BoundValidator(const T &lower_bound,
                   const T &upper_bound) : lbound_(lower_bound),
                                           ubound_(upper_bound)
    {
    }

    __host__ __device__ bool operator()(T k)
    {
  ...
  struct NotSortedAscendingly
  {
    ...
    NotSortedAscendingly(Container &rv, const size_t &sz) : ptr_(&rv[0]),
                                                            sz_(sz)
    {
    }

    __host__ __device__ bool operator()(VType k)
    {
      if (k + 1 < sz_)
