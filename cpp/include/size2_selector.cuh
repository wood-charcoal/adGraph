    ...
    Vector<IndexType> &aggregates, int &num_aggregates)
{
  ...
  //  Hipsparse::csr2coo( n, nnz, A_row_offsets_ptr, row_indices.raw()); // note : amgx uses cusp for that
  // hipsparseHandle_t cusp_handle;
  ...
  { /* comment out by Tin-Yin
    ...
    Hipsparse::csrmv(false, false, n, n, nnz,&alpha,A_nonzero_values_ptr, A_row_offsets_ptr, A_column_indices_ptr, ones.raw(),&beta, row_sum.raw());
