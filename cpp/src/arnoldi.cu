...
namespace nvgraph
{
        ...
        : m_A(A), m_markov(false), m_laplacian(false), m_tolerance(1.0E-12), m_iterations(0), m_dirty_bit(false), m_max_iter(500), has_init_guess(false)
    {
        ...
        //    hipsparseHandle_t t1 = Hipsparse::get_handle();
        //    hipblasHandle_t t2 = Hipblas::get_handle();
...
// we can print stats after setup to have the initial residual
    ...
    void ImplicitArnoldi<IndexType_, ValueType_>::setup(Vector<ValueType> &initial_guess, const int restart_it, const int nEigVals)
    {
        ...
        if (!has_init_guess)
        {
            ...
            Hipblas::scal(n, (ValueType_)1.0 / normQ1, m_V.raw(), 1);
        }
        else
        {
        ...
        if (m_laplacian)
        {
            ...
            //     &beta,
            //     m_D.raw());
            Hipblas::scal(nnz, gamma, m_A.get_raw_values(), 1);
        ...
        // normalize
        Hipblas::scal(n, (ValueType_)1.0 / Hipblas::nrm2(n, m_Vi[0], 1), m_Vi[0], 1);
        m_iterations = 0;
        // arnoldi from 0 to k
    ...
    bool ImplicitArnoldi<IndexType_, ValueType_>::solve_arnoldi(int lower_bound, int upper_bound)
    {
        ...
        for (int i = lower_bound; i < upper_bound; ++i)
        {
            ...
            {
                m_beta = Hipblas::nrm2(n, m_Vi[i], 1);
                // Vi = Vi/||Vi||
                Hipblas::scal(n, (ValueType_)1.0 / m_beta, m_Vi[i], inc);
                // m_V.dump((i-1)*n,n);
            }
            ...
            if (m_markov)
            {
                Hipblas::scal(n, m_damping, m_Vi[i + 1], inc);
                Hipblas::dot(n, m_a.raw(), inc, m_Vi[i], inc, &dot_res);
                Hipblas::axpy(n, dot_res, m_b.raw(), inc, m_Vi[i + 1], inc);
            }
            ...
            {
                // H(j,i) = AVi.Vj
                Hipblas::dot(n, m_Vi[i + 1], inc, m_Vi[j], inc, &Hji);
                m_H[i * m_krylov_size + j] = Hji;
                // V(i + 1) -= H(j, i) * V(j)
                Hipblas::axpy(n, -Hji, m_Vi[j], inc, m_Vi[i + 1], inc);
            }
            if (i > 0)
            {
            ...
            //||Vi+1||
            m_beta = Hipblas::nrm2(n, m_Vi[i + 1], 1);
            if (i + 1 < upper_bound)
            {

                Hipblas::scal(n, (ValueType_)1.0 / m_beta, m_Vi[i + 1], inc);
            }
    ...
    bool ImplicitArnoldi<IndexType_, ValueType_>::solve_it()
    {
        ...
        if (m_residual < m_tolerance)
            return true; // no need to do the k...p arnoldi steps
    ...
    void ImplicitArnoldi<IndexType_, ValueType_>::implicit_restart()
    {
        ...
        if (!m_miramns)
            std::copy(m_H.begin(), m_H.end(), m_H_select.begin());
        ...
        // optim:  avoid the cpy here
        if (!m_miramns)
            std::copy(m_H_select.begin(), m_H_select.end(), m_H.begin());
    }
    ...
    void ImplicitArnoldi<IndexType_, ValueType_>::refine_basis()
    {
        ...
        hipMemcpyAsync(m_Q_d.raw(), &m_Q[0], (size_t)(m_select * m_select * sizeof(m_Q[0])), hipMemcpyHostToDevice);
        hipCheckError();
        hipMemcpyAsync(fptr, m_Vi[nk], (size_t)(n * sizeof(ValueType_)), hipMemcpyDeviceToDevice);
        hipCheckError();
        ...
        if (m_select != m_krylov_size)
            Hipblas::scal(n, m_beta, fptr, 1);

        Hipblas::scal(n, alpha, fptr, 1);

        alpha = m_H_select[(nev - 1) * nk + nev];

        Hipblas::gemm(false, false, n, 1, nk, &alpha, m_V.raw(), n, m_Q_d.raw(), nk, &beta, fptr, n);
    ...
    void ImplicitArnoldi<IndexType_, ValueType_>::compute_eigenvectors()
    {
        ...
        hipCheckError();
        Hipblas::gemm(false, false, n, nev, nk, &alpha, m_V.raw(), n,
                     m_ritz_eigenvectors_d.raw(), nk,
                     &beta, m_eigenvectors.raw(), n);
        // nrm 1 for pagerank
        if (m_markov)
            Hipblas::scal(n, (ValueType_)1.0 / m_eigenvectors.nrm1(), m_eigenvectors.raw(), 1);
