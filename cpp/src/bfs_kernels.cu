...
namespace bfs_kernels
{
										 ...
										 int *edge_mask)
	{
			 ...
			 block_off += blockDim.x * gridDim.x)
		{
			...
			BlockDiscontinuity(discontinuity_temp_storage).FlagHeads(is_head_a, visited_bmap_index, hipcub::Inequality());
			...
			int local_bmap_agg =
				WarpReduce(reduce_temp_storage).HeadSegmentedReduce(local_visited_bmap, is_head, BitwiseAnd());
												  ...
												  int *edge_mask)
	{
			 ...
			 idx += gridDim.x * logical_warps_per_block)
		{
				 ...
				 i_edge += BOTTOM_UP_LOGICAL_WARP_SIZE)
			{
				...
				unsigned int logical_warp_valid_p_ballot = warp_valid_p_ballot >> (BOTTOM_UP_LOGICAL_WARP_SIZE * logical_warp_id_in_warp);
	...
	{
		IndexType end = ((total_degree - 1 + TOP_DOWN_EXPAND_DIMX) / TOP_DOWN_EXPAND_DIMX * NBUCKETS_PER_BLOCK + 1);
								...
								hipStream_t m_stream)
	{
		...
		grid.x = min((IndexType)MAXBLOCKS,
					 ((total_degree - 1 + TOP_DOWN_EXPAND_DIMX) / TOP_DOWN_EXPAND_DIMX * NBUCKETS_PER_BLOCK + 1 + block.x - 1) / block.x);
										  ...
										  bool directed)
	{
		...
		//
		__shared__ IndexType shared_local_new_frontier_candidates[TOP_DOWN_BATCH_SIZE * TOP_DOWN_EXPAND_DIMX];
		__shared__ IndexType shared_local_new_frontier_predecessors[TOP_DOWN_BATCH_SIZE * TOP_DOWN_EXPAND_DIMX];
		__shared__ IndexType block_n_frontier_candidates;

		IndexType block_offset = (blockDim.x * blockIdx.x) * max_items_per_thread;
		IndexType n_items_per_thread_left = (totaldegree - block_offset + TOP_DOWN_EXPAND_DIMX - 1) / TOP_DOWN_EXPAND_DIMX;
			 ...
			 n_items_per_thread_left -= MAX_ITEMS_PER_THREAD_PER_OFFSETS_LOAD)
		{
			...
			if (threadIdx.x < (nitems_per_thread * NBUCKETS_PER_BLOCK + 1))
				shared_buckets_offsets[threadIdx.x] =
					frontier_degrees_exclusive_sum_buckets_offsets[block_offset / TOP_DOWN_BUCKET_SIZE + threadIdx.x];
			...
			while (left < nitems_per_thread)
			{
				...
				IndexType nvalues_to_load = shared_buckets_offsets[right * NBUCKETS_PER_BLOCK] - shared_buckets_offsets[left * NBUCKETS_PER_BLOCK] + 1;
				...
				while (nvalues_to_load > (TOP_DOWN_EXPAND_DIMX + 1))
				{
					...
					nvalues_to_load = shared_buckets_offsets[right * NBUCKETS_PER_BLOCK] - shared_buckets_offsets[left * NBUCKETS_PER_BLOCK] + 1;
				}

				IndexType nitems_per_thread_for_this_load = right - left;

				IndexType frontier_degrees_exclusive_sum_block_offset = shared_buckets_offsets[left * NBUCKETS_PER_BLOCK];
				...
				if (nvalues_to_load == (TOP_DOWN_EXPAND_DIMX + 1) && threadIdx.x == 0)
				{
					shared_frontier_degrees_exclusive_sum[TOP_DOWN_EXPAND_DIMX] =
						frontier_degrees_exclusive_sum[frontier_degrees_exclusive_sum_block_offset + TOP_DOWN_EXPAND_DIMX];
				}
					 ...
					 item_index += TOP_DOWN_BATCH_SIZE)
				{
					...
					IndexType current_max_edge_index = min(block_offset + (left + nitems_per_thread_for_this_load) * blockDim.x,
														   totaldegree);
...
#pragma unroll
					for (IndexType iv = 0; iv < TOP_DOWN_BATCH_SIZE; ++iv)
					{
						...
						if (gid < current_max_edge_index)
						{
							...
							IndexType bucket_start = shared_buckets_offsets[start_off_idx] - frontier_degrees_exclusive_sum_block_offset;
							IndexType bucket_end = shared_buckets_offsets[start_off_idx + 1] - frontier_degrees_exclusive_sum_block_offset;
...
#pragma unroll
					...
					BlockScan(scan_storage).ExclusiveSum(thread_n_frontier_candidates, thread_frontier_candidate_offset);
...
#pragma unroll
					for (int iv = 0; iv < TOP_DOWN_BATCH_SIZE; ++iv)
					{
						...
						if (idx_shared < block_n_frontier_candidates)
						{
							...
							if (!(m & q))
							{ // if this thread was the first to discover this node
								if (distances)
									distances[v] = lvl;
...
#pragma unroll
						 ...
						 bool deterministic)
	{
		...
		dim3 grid;
		grid.x = min((totaldegree + max_items_per_thread * block.x - 1) / (max_items_per_thread * block.x),
					 (IndexType)MAXBLOCKS);
												  ...
												  IndexType *nisolated)
	{
		typedef hipcub::BlockLoad<IndexType, FLAG_ISOLATED_VERTICES_DIMX,
								  FLAG_ISOLATED_VERTICES_VERTICES_PER_THREAD, hipcub::BLOCK_LOAD_WARP_TRANSPOSE>
			BlockLoad;
		typedef hipcub::BlockStore<IndexType, FLAG_ISOLATED_VERTICES_DIMX,
								   FLAG_ISOLATED_VERTICES_VERTICES_PER_THREAD, hipcub::BLOCK_STORE_WARP_TRANSPOSE>
			BlockStore;
		...
		__shared__ typename WarpReduce::TempStorage warp_reduce_temp_storage[FLAG_ISOLATED_VERTICES_DIMX / FLAG_ISOLATED_VERTICES_THREADS_PER_INT];
