...
// --- Start of Graph Generation Function= ---
                               ...
                               std::vector<float> &weights_h)
{
    ...
    while (edges.size() < nnz)
    {
        ...
        if (src != dst)
            // Changed the condition to properly track unique edges: (src, dst)
            if (edge_set.find({src, dst}) == edge_set.end())
            {
                // CSC format stores (destination, source)
...
#ifdef DEBUG
    // Only print debug output on root rank
    int rank = 0;
...
int main(int argc, char **argv)
{
    ...
    // --- Graph Generation ---
    // All ranks generate the same full graph data
    ...
    // Initial bookmark/personalization vector
